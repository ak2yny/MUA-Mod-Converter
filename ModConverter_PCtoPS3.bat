...
REM Mod Converter settings:
REM Convert textures if necessary (yes =true; no =false)
set MCTextrs=true
...
echo Data folder not found. Check the mod's integrity.
goto Errors
:ModConvert2
set "CM=%pathonly:~,-1% - for %ForPltfrm%"
for /f "delims=" %%h in ('dir /b /s herostat*.txt herostat*.xml') do set "hs=%%~h"
CLS
...
goto ModConvert4
:MC3pc
robocopy /s "%pathonly:~,-1%" "%CM%"
:ModConvert4
cd "%CM%"
for /f "delims=" %%s in ('dir /s /b sounds\*.zsm sounds\*.zss') do (
  cd "%%~dps"
  set oldjson=
  set "fullpath=%%~s"
  call :filesetup
...
:updateCFG
REM added custom filter for mod conversion (findstr), could add for maps: automaps dialogs maps motionpaths shaders skybox, maybe actors hud mannequins
for /f %%a in ('dir /ad /b "%pathonly%" ^| findstr /i "conversations data effects models scripts textures"') do for /f "delims=" %%i in ('dir /a-d /b /s "%pathonly%%%~a"') do set "fi=%%~dpni" & call :writeNewFiles %%~xi %%~ni
...
REM loose files not supported for now
REM if /i "%e%"==".xmlb" set format=xml_resident & goto toCFG
call :checkCBI
...
EXIT /b
:actors.igb
for /f "delims=0123456789" %%i in ("%1") do set "format=actoranimdb" & EXIT /b 0
...
:ReadHSlxml attribute_name herostat_variable
call set "h=%%%2%%"
for /f "tokens=1* skip=2 delims==" %%a in ('find /i " %1 = " "%h%"') do set "val=%%b" & goto RHSlFix
EXIT /b 1
:RHSlFix
set "%1=%val:~1,-2%"
EXIT /b 0
